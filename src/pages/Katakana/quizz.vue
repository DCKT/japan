  ...
  <div class="container" v-bind:class="formClass">
    ...
    <div v-if="end">
      ...
      <div class="percentage">
        {{ Math.round((state.found.length * 100 / state.katakana.length) * 100) / 100 }} %
      </div>
        ...
        <div class="col-md-6">
          <div class="panel panel-success">
            ...
            <div v-for="found in state.found"  class="result">
              <span class="katakana">{{found.katakana}}</span>
        ...
        <div class="col-md-6">
          <div class="panel panel-danger">
            ...
            <div v-for="failed in state.failed" class="result">
              <span class="katakana">{{failed.katakana}}</span>
    ...
    <div v-else>
      <h1>{{ $t('quizz.title', { type: 'katakana' }) }} !</h1>
      <div>{{ state.currentQuizz }} / {{ state.katakana.length }}</div>
      <div class="katakana-quizz">
        {{ state.selected.katakana }}
      </div>
...
export default {
  data () {
    return {
      ...
      state: KatakanaStore.state,
    }
  },
  methods: {
    submit() {
      const isValid = KatakanaStore.check(this.romaji);

      if (isValid) {
        this.valid();
        this.end = KatakanaStore.addToFound();
      }
      else {
    ...
    pass() {
      this.end = KatakanaStore.passCurrentKatakana();
    },
    retry() {
